...
def register(request):
    if request.method == "POST":
        ...
        if not all([firstname.strip(), lastname.strip(), email.strip(), phone.strip(), password.strip(), cpassword.strip()]):
        ...
        # Additional check for empty fields after stripping whitespaces
        if not any([firstname, lastname, email, phone, password, cpassword]):
            messages.error(request, "All fields cannot contain only spaces.")
            return HttpResponseRedirect(request.path_info)
...
def editcoupon(request,id):
    ...
    return render(request,'editcoupon.html',{'editpro':coupon})
...
def checkoutpage(request):
    ...
    if not cart_items.exists():
        messages.warning(request, 'Your cart is empty. Add products to the cart before checkout.')
        return redirect('cart')
...
def render_to_pdf(request,order_item_id):
...
def order_invoice(request, order_item_id):
    order_item = get_object_or_404(OrderItem, id=order_item_id)
    ...
    return render(request, 'downloadinvoice.html', context)
